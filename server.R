# Please take note of any code chunks commented out, replace this code with your calculations
# The code already handles aggregation and mutation of duplicate rows of BrCrCode. 
# You only need to create the calculations, refer to a person who is proficient in R-language
# if you need assistance with making formulas.

# Guideline:
# Store a result in a variable. You can name it nearly anything you like, but cannot start with a number or punctuation.
# survivalRate <- 100 - 50
# You can also use other variable names in your formulas
# makeHundred <- survivalRate * 2
# Stringing them together can make elaborate formulas. Here's one from ShinyPSASorter
...
# number variables with a as.numeric().
# nonPsa becomes as.numeric(nonPsa)

# Object '(whatever)' not found:
# This is tricky. It's implying that at some point in the code it's unable to find the variable name you are looking for.
# Backtrack your code, see where you enter certain functions, and with what variables.
# Refer to your resident R-language professional if you need assistance.
# enterFunction <- function(variableOne){
#   print(variableTwo)  
# }
...
shinyServer(function(input, output, session) {
  ...
  meatAndBeans <- function(dat){
    ...
    testMothers <- testFathers <- testParentsVerbaitum <- testParents # <- aggregate(cbind(as.numeric(Planted, nonPsa, PsaDeaths)) ~ BrCrCode+MotherCode+FatherCode, data = testParents, FUN = sum) #omitted is +survivalrate
